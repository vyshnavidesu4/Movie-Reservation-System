...
const SeatSelection = ({ bookingSelection, onSeatsSelect, onBack, movieId }) => {
  ...
  useEffect(() => {
    ...
    const loadShowtime = async () => {
      try {
        const res = await axios.post("http://localhost:5000/api/showtime/getShowtime", {
          movieId,
  ...
  return (
    <div className="seat-selection-page">
      ...
      <header className="header">
        ...
        <h1>
          Select Seats for {movie.title} at {bookingSelection.theatre.name}<br />
